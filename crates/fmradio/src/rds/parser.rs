//! RDS Protocol Parser and Block Synchronization
//!
//! This module implements the RDS (Radio Data System) protocol parser, which handles:
//! - Block synchronization using syndrome matching (similar to redsea's block_sync.cc)
//! - Forward Error Correction (FEC) for 1-2 bit burst errors
//! - RDS group decoding (0A/0B, 1A, 2A/2B, 3A, 4A, 10A, 14A, etc.)
//! - Station metadata extraction (PS, RT, PTY, TP, TA, AF, DI flags, etc.)
//!
//! References:
...
pub fn rds_syndrome(word26: u32) -> u16 {
...
pub fn rds_offset_for_syndrome(s: u16) -> Option<char> {
    if s == SYNDROME_A {
        Some('A')
...
pub fn rds_data_from_word(word26: u32) -> u16 {
...
pub struct RdsParser {
    ...
    pub(super) shift_len: usize,
    /// Program Service name buffer (8 chars)
    pub(super) ps: [u8; 8],
    pub(super) ps_received_mask: u8, // bit mask of which 2-char segments received (4 segments)
    ...
    pub(super) rt: Vec<u8>,
    pub(super) rt_received_mask: Vec<bool>,
    /// Enable verbose debug output
    pub(super) verbose: bool,
    /// Station metadata extracted from RDS
    pub(super) station_info: StationInfo,
